import * as XLSX from "xlsx";
import { StudentScore } from "@/types/score";

/* ===== BORDER HELPER ===== */
const allBorder = {
  top: { style: "thin" },
  bottom: { style: "thin" },
  left: { style: "thin" },
  right: { style: "thin" },
};

interface ExportMeta {
  className: string;
  schoolYear: string;
  semester: number;
}

export function exportScoreExcel(
  students: StudentScore[],
  meta: ExportMeta
) {
  if (!students || students.length === 0) return;

  /* ===== 1. DATA ===== */
  const titleRow = [`BẢNG ĐIỂM LỚP ${meta.className}`];
  const schoolYearRow = [`Năm học: ${meta.schoolYear}`];
  const semesterRow = [`Học kỳ: ${meta.semester}`];
  const emptyRow: never[] = [];

  const headerRow = ["STT", "Họ và tên", "Điểm TB", "Xếp loại"];

  const bodyRows = students.map((s, index) => [
    index + 1,
    s.studentName,
    s.averageScore ?? "",
    s.grade ?? "",
  ]);

  const sheetData = [
    titleRow,
    schoolYearRow,
    semesterRow,
    emptyRow,
    headerRow,
    ...bodyRows,
  ];

  /* ===== 2. CREATE SHEET ===== */
  const ws = XLSX.utils.aoa_to_sheet(sheetData);

  /* ===== 3. MERGE TITLE ===== */
  ws["!merges"] = [
    { s: { r: 0, c: 0 }, e: { r: 0, c: 3 } },
    { s: { r: 1, c: 0 }, e: { r: 1, c: 3 } },
    { s: { r: 2, c: 0 }, e: { r: 2, c: 3 } },
  ];

  /* ===== 4. TITLE STYLE ===== */
  ["A1", "A2", "A3"].forEach((cellRef, idx) => {
    const cell = ws[cellRef];
    if (!cell) return;

    cell.s = {
      font: {
        bold: idx === 0,
        sz: idx === 0 ? 16 : 12,
      },
      alignment: {
        horizontal: "center",
        vertical: "center",
      },
    };
  });

  /* ===== 5. HEADER STYLE ===== */
  const headerRowIndex = 4; // zero-based
  ["A", "B", "C", "D"].forEach((col) => {
    const cell = ws[`${col}${headerRowIndex + 1}`];
    if (!cell) return;

    cell.s = {
      font: { bold: true },
      alignment: {
        horizontal: "center",
        vertical: "center",
      },
      border: allBorder,
    };
  });

  /* ===== 6. BODY STYLE + ALL BORDER ===== */
  const startBodyRow = headerRowIndex + 2;
  const endBodyRow = startBodyRow + bodyRows.length - 1;

  for (let r = startBodyRow; r <= endBodyRow; r++) {
    ["A", "B", "C", "D"].forEach((col, idx) => {
      const cell = ws[`${col}${r}`];
      if (!cell) return;

      cell.s = {
        alignment: {
          horizontal: idx === 1 ? "left" : "center",
          vertical: "center",
        },
        border: allBorder,
      };
    });
  }

  /* ===== 7. COLUMN WIDTH ===== */
  ws["!cols"] = [
    { wch: 6 },   // STT
    { wch: 28 },  // Họ tên
    { wch: 16 },  // Điểm TB
    { wch: 14 },  // Xếp loại
  ];

  /* ===== 8. ROW HEIGHT ===== */
  ws["!rows"] = [
    { hpt: 30 }, // BẢNG ĐIỂM
    { hpt: 22 }, // NĂM HỌC
    { hpt: 22 }, // HỌC KỲ
    { hpt: 10 }, // empty
    { hpt: 26 }, // header
    ...students.map(() => ({ hpt: 24 })),
  ];

  /* ===== 9. EXPORT ===== */
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "BangDiem");

  XLSX.writeFile(
    wb,
    `Bang_diem_${meta.className}_HK${meta.semester}.xlsx`
  );
}
